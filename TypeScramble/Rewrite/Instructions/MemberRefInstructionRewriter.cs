    ...
    class MemberRefInstructionRewriter : InstructionRewriter<MemberRef> {
        public override void ProcessOperand(ITypeService service, MethodDef method, IList<Instruction> body, ref int index, MemberRef operand) {
            ...
            if (body[index].OpCode == OpCodes.Newobj) {
                ApplyObjectCreationProxy(service, currentMethod, body, ref index, operand);
            } else {
                ApplyCallProxy(service, currentMethod, body, ref index, operand);
            }

        }

        private void ApplyObjectCreationProxy(ITypeService service, ScannedItem parentItem, IList<Instruction> body, ref int index, MemberRef operand) {
        ...
        private void ApplyCallProxy(ITypeService service, ScannedItem parentItem, IList<Instruction> body, ref int index, MemberRef operand) {
